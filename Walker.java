...
public class Walker {
     ...
     */
    private osrs.Walker.Type getType(String s) {
        if (s.matches("(?i)((.*)?door)")) {
            return osrs.Walker.Type.DOOR;
        } else if (s.matches("(?i)(gate(.*)?)")) {
            return osrs.Walker.Type.GATE;
        } else if (s.equalsIgnoreCase("ladder")) {
            return osrs.Walker.Type.LADDER;
        }
        return osrs.Walker.Type.TEAPOT;
    }
